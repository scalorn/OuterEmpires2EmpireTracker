...
- [ ] 6. SQLite Backend — CRUD Implementation
  ...
  - [x] 6.4 SQLite CRUD: PlayerProfile (with Skills child table), DeliveryRoute (with Stops child table), DeliveryPlan (with Stops and Items child tables)
    ...
    - Verification: getDiagnostics — compiles cleanly
  - [x] 6.5 SQLite CRUD: Ship (with Components + Items), ShipTemplate (with Components), MarketListing, MarketTransaction
    ...
    - Verification: getDiagnostics — compiles cleanly
  - [x] 6.6 SQLite CRUD: PricingPlan (with Prices), BuildPlan (with Items), StockPlan (with Targets), StockProfile (with Entries), SupplyChain (with Stages), Asteroid (with Reserves), Station (with Components + Items)
