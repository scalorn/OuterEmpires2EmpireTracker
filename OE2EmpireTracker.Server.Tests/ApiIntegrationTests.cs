...
public class DataStorageTests
{
    ...
    public void Setup()
    {
        ...
        // Create a character token for data tests
        var createResponse = _ownerClient
            .PostAsJsonAsync("/api/v1/tokens", new { characterName = "DataTestChar" })
        ...
        var otherResponse = _ownerClient
            .PostAsJsonAsync("/api/v1/tokens", new { characterName = "OtherChar" })
            .GetAwaiter().GetResult();
    ...
    [Test]
    [Order(1)]
    public async Task Put_Character_Data_Uploads_Entity()
    {
        using var charClient = _factory.CreateAuthenticatedClient(_charToken);
        var colonyData = JsonSerializer.Serialize(
            new { UUID = "colony-001", Name = "TestColony", Population = 100 });
        var content = new StringContent(colonyData, Encoding.UTF8, "application/json");
    ...
    [Test]
    [Order(2)]
    public async Task Get_Character_Data_Returns_Uploaded_Entity()
    {
        using var charClient = _factory.CreateAuthenticatedClient(_charToken);

        var response = await charClient.GetAsync(
            $"/api/v1/characters/{_charUUID}/data/Colonies/colony-001");
        Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.OK));

        var json = await response.Content.ReadFromJsonAsync<JsonElement>();
        Assert.That(json.GetProperty("Name").GetString(), Is.EqualTo("TestColony"));
    }

    [Test]
    [Order(3)]
    public async Task Character_Cannot_Access_Other_Characters_Data()
    {
        using var charClient = _factory.CreateAuthenticatedClient(_charToken);

        var response = await charClient.GetAsync(
            $"/api/v1/characters/{_otherCharUUID}/data/Colonies");
        Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.Forbidden));
    }

    [Test]
    [Order(4)]
    public async Task Owner_Can_Access_Any_Characters_Data()
    {
