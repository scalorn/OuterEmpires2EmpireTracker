...
Build delivery routes between colonies, stations, and asteroids, then auto-fill plans based on what each location actually needs — commodities, flatpacks, resources, workers. Each stop has a purpose (Cargo, Refuel, or both) and an optional fuel estimate. The Delivery Execution form walks you through each stop with checkboxes — when you check off a delivery at a station, the station's inventory updates automatically. Assign a ship to any delivery plan to see cargo volume and mass at a glance — the form warns you in red when you're over capacity and offers to split the load into multiple trips automatically. The flatpack auto-fill now supports a time horizon — set it to only include structures that will finish building within a certain number of hours, so you're not hauling flatpacks for builds that won't need them for days.
...
### Ship Templates
Design reusable ship configurations. Open from **Manage → Ship Templates**. Pick a hull, then fill component slots (reactors, drives, weapons, cargo pods, shields, etc.) from your blueprint collection. Each slot has a filtered combo box — type part of a blueprint name to narrow the list instantly. The stats panel updates live as you install components — see mass, power balance, cargo capacity, defence ratings, propulsion, and jump range at a glance. The enhanced stats panel shows derived performance metrics matching the in-game ship info: acceleration factor, turn rate, jump fuel range, shield uptime, weapon sustainability per type, and mining laser sustainability — so you can optimize power budgets before committing to a build. Select a pricing plan to see the total estimated build cost — the price sums hull and all component blueprint costs using your plan's resource prices and time-cost parameters, updating live as you swap parts. When you're ready to build, click Order Build to generate manufacturing items for the hull and every component, feeding directly into the Build Planner.
...
### Supply Chains
Model your resource pipelines from mine to market. Open from **Manage → Supply Chains**. Define a chain of stages — Mine, AsteroidMine, PickUp, Refine, Research, Deliver — each at a colony, station, asteroid, or ship. Location, resource, and route pickers all have inline filtering for quick selection. Set accumulation thresholds on stages so the background processor knows when to trigger deliveries. Assign delivery routes to threshold stages. Pause and resume chains with the Active toggle (inactive chains show grayed out in the list and are skipped by background processing). A flow summary shows the full pipeline at a glance.
