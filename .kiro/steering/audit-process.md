...
9. **UTF-8 Encoding** (utf8-check.js) — Scans all .cs files in OE2EmpireTracker/ and OE2EmpireTracker.Tests/, plus all .md files in spec/ and docs/. Detects invalid UTF-8 byte sequences (U+FFFD replacement characters) and double/triple-encoded UTF-8 mojibake (e.g. em-dash U+2014 becoming garbled multi-byte sequences like `\xC3\xA2\xE2\x82\xAC\xE2\x80\x94`). These occur when UTF-8 bytes are misinterpreted as Windows-1252 and re-encoded.
...
16. **Inline Sort** (inline-sort-check.js) — Detects inline LINQ OrderBy/OrderByDescending calls on data model collections that should use CollectionSortHelper instead.
17. **Flatpack Integrity** (flatpack-check.js) — Cross-checks flatpack definitions in BaselineData.json against blueprint types and colony structures.
18. **Code Metrics** (code-metrics.js) — Parses OE2EmpireTracker/OE2EmpireTracker.Metrics.xml (generated by MSBuild /t:Metrics) and reports methods or types exceeding configurable quality thresholds. Checks MaintainabilityIndex (min: 20), CyclomaticComplexity (max: 25), ClassCoupling (max: 80), and DepthOfInheritance (max: 6). Known violations are tracked in a baseline file (.kiro/tools/code-metrics-baseline.json). Gracefully skips if the XML file is missing.
19. **Mutation Audit** (mutation-audit.js) — Detects direct entity mutation outside service classes. Checks three categories: (1) WriteContext() calls in files not on the allowed service/context list, (2) direct entity property sets (e.g. ._colony.X =, .Data.X =) in Form*.cs files that bypass ViewModels, and (3) the same patterns in *ViewModel.cs files that bypass services. Skips Designer.cs files, comments, string literals, ViewModel field writes, property declarations, and local variable declarations.
20. **DateTime Check** (datetime-check.js) — Detects direct `DateTime.UtcNow` usage outside SystemClock.cs. All code (production and test) must use `SystemClock.UtcNow` so that tests can freeze time deterministically. The only allowed file is SystemClock.cs itself (which provides the default implementation).
