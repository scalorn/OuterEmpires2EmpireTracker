...
const itemColumns: GridColumn<PricingPlanItem>[] = [
  { key: 'itemName', header: 'Item Name', type: 'readonly' },
  { key: 'itemType', header: 'Item Type', type: 'readonly' },
...
export function PricingPlanForm() {
  ...
  const filteredPlans = useMemo(() => {
    if (!plans) return [];
    if (!filterText.trim()) return plans;
  ...
  const handleRowChange = useCallback((index: number, row: PricingPlanItem) => {
    setForm((prev) => {
    ...
    <div className="flex h-full flex-col">
      <div className="border-b border-gray-700 p-3">
        ...
        <input
          ...
          className="mt-2 w-full rounded border border-gray-600 bg-gray-700 px-3 py-1.5 text-sm text-white placeholder-gray-400"
        />
      </div>
      {filteredPlans.length === 0 ? (
        <EmptyState title="No pricing plans" message="Create a new pricing plan to get started." />
      ) : (
