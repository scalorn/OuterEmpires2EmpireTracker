...
**Status: Complete**
All forms with editable controls now write to the data model immediately on change. Save handlers simplified to only call `writeContext()` via ViewModel.Save().

### FormColony.cs
- [x] `txtPlanetName` → add TextChanged handler writing to `colonyViewModel.PlanetName`
- [x] `txtColonyName` → add TextChanged handler writing to `colonyViewModel.ColonyName`
- [x] `txtSystemName` → add TextChanged handler writing to `colonyViewModel.Data.SystemName`
- [x] Remove duplicate reads from `cmdSave_Click` (keep only `colonyViewModel.Save()`)

### FormBlueprint.cs
- [x] `txtName` → add TextChanged handler writing to `viewModel.Name`
- [x] `txtNickName` → add TextChanged handler writing to `viewModel.NickName`
- [x] `txtDescription` → add TextChanged handler writing to `viewModel.Description`
- [x] `txtCopyCost` → add TextChanged handler writing to `viewModel.CopyCost`
- [x] `cmbBlueprintType` → write to `viewModel.BluePrintType` on SelectedIndexChanged
- [x] `cmbShipClass` → write to `viewModel.Class` on SelectedIndexChanged
- [x] `cmbTechLevel` → write to `viewModel.TechLevel` on SelectedIndexChanged
- [x] `cmbEvolution` → write to `viewModel.Evolution` on SelectedIndexChanged
- [x] `cmbBaseBlueprint` → write to `viewModel.BaseBlueprintUUID` on SelectedIndexChanged
- [x] `chkGlobalBlueprint` → write on CheckedChanged
- [x] Remove duplicate reads from `btnSave_Click`

### FormSurvey.cs
- [x] `txtPlanetName` → add TextChanged handler writing to `viewModel.PlanetName`
- [x] `txtSystemName` → add TextChanged handler writing to `viewModel.SystemName`
- [x] `txtSurveyID` → add TextChanged handler writing to `viewModel.SurveyID`
- [x] `txtNickName` → add TextChanged handler writing to `viewModel.NickName`
- [x] `txtScannedBy` → add TextChanged handler writing to `viewModel.ScannedBy`
- [x] `txtScanDateTime` → add TextChanged handler writing to `viewModel.DateTime`
- [x] `txtSensorAbundance` → add TextChanged handler writing to `viewModel.SensorAbundance`
- [x] `txtPurityModifier` → add TextChanged handler writing to `viewModel.PurityModifier`
- [x] `txtScanLevel` → add TextChanged handler writing to `viewModel.ScanLevel`
- [x] `cmbScannerBlueprint` → write to `viewModel.ScannerBlueprintUUID` on SelectedIndexChanged
- [x] Remove duplicate reads from `btnSave_Click`

### FormPlayerProfile.cs
- [x] `txtPlayerName` → add write to `viewModel.Name` in existing TextChanged handler
- [x] `txtTotalCredits` → add TextChanged handler writing to `viewModel.TotalCredits`
- [x] `txtSkillPoints` → add TextChanged handler writing to `viewModel.SkillPoints`
- [x] `txtPublicRank`, `txtPublicRankCurXP`, `txtPublicRankNextXP` → add TextChanged handlers
- [x] `txtPrivateRank`, `txtPrivateRankCurXP`, `txtPrivateRankNextXP` → add TextChanged handlers
- [x] `txtMilitaryRank`, `txtMilitaryRankCurXP`, `txtMilitaryRankNextXP` → add TextChanged handlers
- [x] `cmbFaction` → write to `viewModel.Faction` on TextChanged/SelectedIndexChanged
- [x] Remove duplicate reads from `cmdSave_Click`

### FormDeliveryRoute.cs
- [x] `txtRouteName` → add TextChanged handler writing to `viewModel.Name`
- [x] `txtPlanName` → add write to `planViewModel.Data.Name` in existing TextChanged handler
- [x] Remove duplicate reads from `cmdSave_Click`
