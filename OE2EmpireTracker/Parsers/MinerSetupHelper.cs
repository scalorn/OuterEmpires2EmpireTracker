...
namespace OE2EmpireTracker.Parsers
{
    ...
    public static class MinerSetupHelper
    {
            ...
            decimal> maxRates)
        {
            ...
            Log.Info(
                "SetupMiners starting for colony {0}: {1} structures, {2} maxRate entries",
                colony.PlanetName,
            ...
            var defaultSurveysBefore = playerContext.SurveyList
                ...
                .ToList();
            Log.Info(
                "SetupMiners: {0} DEFAULT surveys for planet '{1}' before processing: [{2}]",
                defaultSurveysBefore.Count,
                colony.PlanetName,
            ...
            foreach (var structure in colony.Structures)
            {
                ...
                Log.Info(
                    "Processing miner {0} (FlatpackBP={1}): MiningSurveyResource='{2}', RefiningResourcePurity='{3}', maxRate={4}, existingSurvey='{5}'",
                    structure.UUID,
            ...
            Log.Info(
                "SetupMiners complete for colony {0}: {1} surveys assigned, {2} timers started, {3} warehouse resources created",
                colony.PlanetName,
                surveysAssigned,
                timersStarted,
                warehouseResourcesCreated);
            ...
            var defaultSurveysAfter = playerContext.SurveyList
                ...
                .ToList();
            Log.Info(
                "SetupMiners: {0} DEFAULT surveys for planet '{1}' after processing: [{2}]",
                defaultSurveysAfter.Count,
                colony.PlanetName,
                string.Join(", ", defaultSurveysAfter.Select(s => s.UUID)));
            if (defaultSurveysAfter.Count > 1)
            {
                Log.Warn(
                    "BL-050 DIAGNOSTIC: Multiple DEFAULT surveys remain for planet '{0}' after SetupMiners! UUIDs: [{1}]",
                    colony.PlanetName,
            ...
            PlayerContext playerContext)
        {
            ...
            {
                Log.Debug(
                    "FindBestSurvey: 0 candidate surveys for planet='{0}', resource='{1}', purity='{2}'",
                    planetName,
                    resourceName,
                    purity);
                return null;
            ...
            Log.Debug(
                "FindBestSurvey: {0} candidate surveys for planet='{1}', resource='{2}', purity='{3}', maxRate={4}",
                candidates.Count,
                planetName,
                resourceName,
                purity,
            ...
            if (maxRate > 0m)
            {
                ...
                var selected = candidates
                    ...
                    .First();
                Log.Info(
                    "FindBestSurvey: selected survey {0} (closest match to maxRate={1}) for {2} ({3})",
                    selected.UUID,
                    maxRate,
                    resourceName,
                    purity);
            ...
            else
            {
                ...
                var selected = candidates
                    ...
                    .First();
                Log.Info(
                    "FindBestSurvey: selected survey {0} (highest amount, maxRate=0) for {1} ({2})",
                    selected.UUID,
                    resourceName,
                    purity);
                return selected;
            ...
            decimal maxRate)
        {
            ...
            if (!string.IsNullOrEmpty(structure.MiningSurvey))
            {
                ...
                if (existing != null)
                {
                    ...
                    {
                        Log.Info(
                            "Preserved existing real survey {0} on structure {1} for {2}",
                    ...
                    if (realSurvey != null)
                    {
                        ...
                        Log.Info(
                            "Upgraded default survey to real survey {0} on structure {1} for {2}",
                            realSurvey.UUID,
                            structure.UUID,
                            resource);
                        return true;
                    ...
                    Log.Info(
                        "Kept default survey {0} on structure {1} for {2} (no real survey available)",
                        structure.MiningSurvey,
                        structure.UUID,
                        resource);
                    return true;
                ...
                Log.Info(
                    "Existing survey {0} no longer exists for structure {1}, reassigning",
                    structure.MiningSurvey,
                    structure.UUID);
            }
            ...
            if (bestSurvey != null)
            {
                ...
                Log.Info(
                    "Assigned real survey {0} to structure {1} for {2}",
                    bestSurvey.UUID,
                    structure.UUID,
                    resource);
                return true;
            ...
            Log.Info(
                "Assigned default survey {0} to structure {1} for {2}",
                defaultSurvey.UUID,
                structure.UUID,
                resource);
            return true;
        ...
        internal static void EnsureWarehouseResource(Colony colony, string resourceName, string purity)
        {
            ...
            if (existing.Count == 0)
            {
                ...
                Log.Info(
                    "Created warehouse resource: {0} ({1}) for colony {2}",
                    resourceName,
                    purity,
                    colony.PlanetName);
            }
        ...
        internal static void SetupTimer(ColonyStructure structure, decimal maxRate)
        {
            ...
            {
                Log.Info(
                    "Skipped timer creation for structure {0}: maxRate is 0 (miner assigned but not actively mining)",
                    structure.UUID);
                return;
            }
            ...
            Log.Info(
                "Created repeating mining timer on structure {0}, next fire in {1}s",
                structure.UUID,
            ...
            EmpireContext empireContext)
        {
            ...
            Log.Info(
                "CleanupDefaultSurvey: expected UUID={0} for planet='{1}', owner='{2}', system='{3}'",
                defaultUUID,
                colony.PlanetName,
                colony.OwnerUUID,
                colony.SystemName);
            ...
            var allDefaults = playerContext.SurveyList
                ...
                .ToList();
            Log.Info(
                "CleanupDefaultSurvey: {0} total DEFAULT surveys for planet '{1}': [{2}]",
                allDefaults.Count,
                colony.PlanetName,
            ...
            foreach (var dup in duplicates)
            {
                ...
                Log.Info(
                    "Removed duplicate default survey {0} for planet {1} (correct UUID is {2})",
                    dup.UUID,
                    colony.PlanetName,
                    defaultUUID);
            }
            ...
            foreach (var key in staleKeys)
            {
                ...
                Log.Info(
                    "Removed stale resource {0} from default survey {1} for colony {2}",
                    key,
                    defaultUUID,
                    colony.PlanetName);
            }
            ...
            if (defaultSurvey.Resources.Count == 0)
            {
                ...
                Log.Info(
                    "Removed empty default survey {0} for colony {1}",
                    defaultUUID,
                    colony.PlanetName);
            }
        }
            ...
            PlayerContext playerContext)
        {
            ...
            Log.Info(
                "CreateOrUpdateDefaultSurvey: looking for UUID={0} (planet='{1}', owner='{2}', system='{3}'), resource={4} ({5}), maxRate={6}",
                uuid,
                colony.PlanetName,
                colony.OwnerUUID,
                colony.SystemName,
            ...
            if (defaultSurvey == null)
            {
                ...
                if (defaultSurvey != null)
                {
                    ...
                    Log.Info(
                        "Found stale default survey {0} for planet {1}, updating UUID to {2}",
                        defaultSurvey.UUID,
                        colony.PlanetName,
                        uuid);
            ...
            if (defaultSurvey != null)
            {
                ...
                Log.Info(
                    "Updated default survey {0} with resource {1} ({2}) amount={3}",
                    uuid,
                    resourceName,
                    purity,
                    amount);
            ...
            else
            {
                ...
                Log.Info(
                    "Created default survey {0} for colony {1} with resource {2} ({3}) amount={4}",
                    uuid,
                    colony.PlanetName,
                    resourceName,
                    purity,
