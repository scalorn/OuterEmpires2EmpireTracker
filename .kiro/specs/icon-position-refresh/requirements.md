...
8. THE `Commodity Industry` property on commodity factory blueprints SHALL be retained for backward compatibility. The property is readable from the BlueprintType Id (the industry name is encoded in the Id), but existing code that reads it from the Properties bag continues to work and is not required to change. Derivation from the Id is optional future work.
