...
- [ ] 1. Core infrastructure — PlayerContext backing fields, properties, and mutation methods for Blueprint, Survey, Colony
  - [ ] 1.1 Convert BlueprintList, SurveyList, ColonyList from public fields to private backing fields with `IReadOnlyList<T>` properties
  ...
  - [ ] 1.2 Add mutation methods for Blueprint (Pattern A: UUID cache + BindingSource + derived caches)
  ...
  - [ ] 1.3 Add mutation methods for Survey (Pattern B: UUID cache + BindingSource)
  ...
  - [ ] 1.4 Add mutation methods for Colony (Pattern B: UUID cache + BindingSource)
    - `AddColony(Colony item)` and `RemoveColony(Colony item)`
    - Lock, inline UUID cache update, `BindingSourceColony?.ResetBindings(false)`
    - _Requirements: 3.1, 3.2, 3.3, 3.4, 3.5, 3.6, 3.7, 5.1, 5.2, 7.4_

- [ ] 2. Remove fallback linear scans from FindBlueprint, FindSurvey, FindColony
  - [ ] 2.1 Remove the `FirstOrDefault` fallback from FindBlueprint
  ...
  - [ ] 2.2 Remove the `FirstOrDefault` fallback from FindSurvey
    - _Requirements: 6.2, 6.4_
  - [ ] 2.3 Remove the `FirstOrDefault` fallback from FindColony
    - _Requirements: 6.3, 6.4_

- [ ] 3. Add UUID caches and Find methods for the 10 entities that don't have them yet
  - [ ] 3.1 Add cache field, Find method, and Invalidate method for PlayerProfile
  ...
  - [ ] 3.2 Add cache, Find, and Invalidate for DeliveryRoute, DeliveryPlan, PricingPlan
    - Same lazy-init dictionary cache pattern for each
    - _Requirements: 5.3, 5.4, 10.1, 10.3_
  - [ ] 3.3 Add cache, Find, and Invalidate for MarketTransaction, StockPlan, StockProfile
    - _Requirements: 5.3, 5.4, 10.1, 10.3_
  - [ ] 3.4 Add cache, Find, and Invalidate for SupplyChain, WarehouseOverflowRule, ExternalCharacter
    - _Requirements: 5.3, 5.4, 10.1, 10.3_

- [ ] 4. Mutation methods for remaining 17 PlayerContext entities
  - [ ] 4.1 Add mutation methods for PlayerProfile (Pattern E: UUID cache + BindingSource)
  ...
  - [ ] 4.2 Add mutation methods for BuildPlan (Pattern D: UUID cache + derived caches)
  ...
  - [ ] 4.3 Add mutation methods for Station, ShipTemplate, Ship, Asteroid, Faction, MarketListing (Pattern C: UUID cache only)
    - Each gets `Add{Entity}` / `Remove{Entity}` with lock and inline cache update
    - _Requirements: 3.1, 3.2, 3.3, 3.4, 3.5, 3.6, 5.1, 5.2_
  - [ ] 4.4 Add mutation methods for DeliveryRoute, DeliveryPlan, PricingPlan, MarketTransaction, StockPlan, StockProfile, SupplyChain, WarehouseOverflowRule, ExternalCharacter (Pattern C: UUID cache only)
    - Each gets `Add{Entity}` / `Remove{Entity}` with lock and inline cache update using the new caches from task 3
    - _Requirements: 3.1, 3.2, 3.3, 3.4, 3.5, 3.6, 5.1, 5.2_
  - [ ] 4.5 Convert remaining 17 PlayerContext list fields to private backing fields with `IReadOnlyList<T>` properties
...
- [ ] 5. Checkpoint — Verify PlayerContext compiles and internal tests pass
  - Ensure all tests pass, ask the user if questions arise.
  - Run `getDiagnostics` on PlayerContext.cs to verify no compile errors
  - Build the solution and run the test suite

- [ ] 6. EmpireContext encapsulation
  - [ ] 6.1 Convert all 9 EmpireContext list fields to private backing fields with `IReadOnlyList<T>` properties
  ...
  - [ ] 6.2 Add mutation methods for GlobalBlueprint (UUID cache, no BindingSource)
    - `AddGlobalBlueprint` / `RemoveGlobalBlueprint` with inline cache update
    - _Requirements: 4.1, 4.2, 4.3, 4.4, 5.5_
  - [ ] 6.3 Add mutation methods for Commodity (name cache with `_commodityLock`)
    - `AddCommodity` / `RemoveCommodity` with `_commodityLock`, inline name cache update
    - _Requirements: 4.1, 4.2, 4.3, 4.4, 5.5_
  - [ ] 6.4 Add mutation methods for BindingSource-only lists (BlueprintType, ShipClass, TechLevel, Evolution, Resource, ResourceGroup, ResourcePurity)
    - Each gets `Add{Entity}` / `Remove{Entity}` with `ResetBindings(false)`
    - No UUID cache, no lock (single-threaded init only)
    - _Requirements: 4.1, 4.2, 4.5_

- [ ] 7. Checkpoint — Verify both contexts compile cleanly
  - Ensure all tests pass, ask the user if questions arise.
...
- [ ] 8. Install FsCheck and write property-based tests
  - [ ]* 8.1 Install FsCheck NuGet packages
    - Add `FsCheck` and `FsCheck.NUnit` to `OE2EmpireTracker.Tests/packages.config`
    - Add assembly references to the test `.csproj`
    - _Requirements: (testing infrastructure)_
  - [ ]* 8.2 Write property test for Property 1: Add-then-Find round trip
  ...
  - [ ]* 8.3 Write property test for Property 2: Remove-then-Find returns null
  ...
  - [ ]* 8.4 Write property test for Property 4: Invalidate-then-Find rebuilds correctly
  ...
  - [ ]* 8.5 Write property test for Property 5: Snapshot independence
    - **Property 5: Snapshot independence**
    - Get snapshot, mutate backing list via Add/Remove, assert snapshot unchanged
    - **Validates: Requirements 12.1**
  - [ ]* 8.6 Write property test for Property 6: Blueprint mutation invalidates derived caches
    - **Property 6: Blueprint mutation invalidates derived caches**
    - Add blueprint, verify CountBlueprintsByType reflects it
    - **Validates: Requirements 13.1**
  - [ ]* 8.7 Write property test for Property 7: BuildPlan mutation invalidates build item indexes
...
- [ ] 9. Migrate callers — PlayerContext entities (forms, parsers, services)
  - [ ] 9.1 Migrate Blueprint callers
  ...
  - [ ] 9.3 Migrate Colony callers
    ...
    - _Requirements: 9.1, 9.2, 9.3, 9.4_
  - [ ] 9.4 Migrate PlayerProfile callers
    - Replace `.PlayerProfileList.Add(` / `.PlayerProfileList.Remove(` with `AddPlayerProfile()` / `RemovePlayerProfile()`
    - _Requirements: 9.1, 9.2, 9.3, 9.4_
  - [ ] 9.5 Migrate DeliveryRoute, DeliveryPlan, PricingPlan callers
    - Replace direct list mutations with `AddDeliveryRoute()` / `RemoveDeliveryRoute()` etc.
    - _Requirements: 9.1, 9.2, 9.3, 9.4_
  - [ ] 9.6 Migrate BuildPlan, ShipTemplate, Ship callers
    - Replace direct list mutations with corresponding Add/Remove methods
    - _Requirements: 9.1, 9.2, 9.3, 9.4_
  - [ ] 9.7 Migrate Station, Asteroid, Faction, ExternalCharacter callers
    - Replace direct list mutations with corresponding Add/Remove methods
    - _Requirements: 9.1, 9.2, 9.3, 9.4_
  - [ ] 9.8 Migrate MarketListing, MarketTransaction, StockPlan, StockProfile, SupplyChain, WarehouseOverflowRule callers
    - Replace direct list mutations with corresponding Add/Remove methods
    - _Requirements: 9.1, 9.2, 9.3, 9.4_

- [ ] 10. Migrate callers — EmpireContext entities
  - [ ] 10.1 Migrate GlobalBlueprintList callers
    - Replace `.GlobalBlueprintList.Add(` / `.GlobalBlueprintList.Remove(` with `AddGlobalBlueprint()` / `RemoveGlobalBlueprint()`
    - _Requirements: 9.1, 9.2, 9.3, 9.4_
  - [ ] 10.2 Migrate CommodityList callers
    - Replace `.CommodityList.Add(` with `AddCommodity()`
    - _Requirements: 9.1, 9.2, 9.3, 9.4_
  - [ ] 10.3 Migrate BlueprintTypeList, ShipClassList, TechLevelList, EvolutionList, ResourceList, ResourceGroupList, ResourcePurityList callers
    - Replace direct list mutations with corresponding Add/Remove methods
    - _Requirements: 9.1, 9.2, 9.3, 9.4_

- [ ] 11. Checkpoint — Verify all production code compiles after migration
  - Ensure all tests pass, ask the user if questions arise.
...
- [ ] 12. Migrate callers — test project
  - [ ] 12.1 Migrate test setup code that adds entities to PlayerContext lists
  ...
  - [ ] 12.2 Migrate test setup code that adds entities to EmpireContext lists
    - Find all `.Add(` calls on EmpireContext list properties in `OE2EmpireTracker.Tests/`
    - Replace with corresponding `Add{Entity}()` calls
    - _Requirements: 11.1, 11.2, 11.3, 11.4_
  - [ ] 12.3 Migrate test cleanup code that removes entities from context lists
